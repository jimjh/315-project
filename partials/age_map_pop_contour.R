...
output$age_vs_pop <- renderPlot({
  ...
  if (input$age_contour == TRUE) {
    ...
    plot(louisiana.blkgrp10, xlim=c(-90.29, -89.84), ylim=c(29.81, 30.10), col=col.vector(agepop.data[['male']]), border=1, lwd=.5)
  ...
  if (input$age_contour == TRUE) {
    ...
    plot(louisiana.blkgrp10, xlim=c(-90.29, -89.84), ylim=c(29.81, 30.10), col=col.vector(agepop.data[['female']]), border=1, lwd=.5)
    rect(-90.13884, 29.98311, -90.06836, 29.92592, border=2, lwd=4)
  }
...
output$zoomed_in_age <- renderPlot({
  par(mfrow=c(1,2))
  plot(louisiana.blkgrp10, xlim=c(-90.13884, -90.06836), ylim=c(29.98311, 29.92592), col=col.vector(agepop.data[['male']]), border=NA)
  title("Zoomed: Male Age Distribution in City Center")
